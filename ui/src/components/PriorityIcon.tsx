...
const priorityConfig: Record<string, { icon: typeof ArrowUp; color: string; label: string }> = {
  critical: { icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault, label: "Критический" },
  high: { icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault, label: "Высокий" },
  medium: { icon: Minus, color: priorityColor.medium ?? priorityColorDefault, label: "Средний" },
  low: { icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault, label: "Низкий" },
};

...
export function ExecutionWorkspaceDetail() {
  ...
  if (error) return <p className="text-sm text-destructive">{error instanceof Error ? error.message : "Не удалось загрузить рабочее пространство"}</p>;
  if (!workspace) return null;

  return (
    <div className="max-w-2xl space-y-4">
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">Рабочее пространство</div>
      ...
      <div className="rounded-lg border border-border p-4">
        ...
        <DetailRow label="Ветка">{workspace.branchName ?? "Нет"}</DetailRow>
        <DetailRow label="Базовый ref">{workspace.baseRef ?? "Нет"}</DetailRow>
        <DetailRow label="Рабочая директория">
          <span className="break-all font-mono text-xs">{workspace.cwd ?? "None"}</span>
        </DetailRow>
        <DetailRow label="Ref провайдера">
        ...
        <DetailRow label="Очистка">
          {workspace.cleanupEligibleAt ? `${new Date(workspace.cleanupEligibleAt).toLocaleString()}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}` : "Не запланирована"}
        </DetailRow>
      </div>
    </div>
  );
